...
public class ExcelUtil<T> {
	...
	public AjaxResult exportExcel(List<T> list, String sheetName) {
		...
		//	@Mark，修改成了2007以后的版本，也就是xlsx格式的文件。
		//	也就不再需要分开到多个Sheet表了。
		//	Excel 2007最大行数为1048576
		//	减去1是为了标题栏的那一行
		int sheetSize = 1048576 - 1;
		//	计算一共有多少个sheet.
		...
		for (int index = 0; index <= sheetNo; index++) {
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				// 写入列名
				cell.setCellValue(
						StringUtils.isEmpty(attr.i18nKey()) ? attr.name() : MessageUtils.message(attr.i18nKey()));
				...
				for (int j = 0; j < fields.size(); j++) {
					...
					try {
						...
						if (attr.isExport()) {
							...
							cell.setCellType(XSSFCell.CELL_TYPE_STRING);
							try {
								//	日期格式单独处理，其它格式基本转字符串就行了
								//	取出值
								Object theValue = field.get(vo);
								//	如果是空值，则填入空字符串结束
								if(theValue == null || "null".equals(String.valueOf(theValue)))
									cell.setCellValue("");
								else {
									//	判断是不是日期类型，Date类
									if(theValue instanceof Date) {
										//	转换为Date对象
										Date date = (Date) theValue;
	...
	public String getfile() throws FileNotFoundException {
		// return RuoYiConfig.getDownloadPath();
		return null;
	}
