...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            //显示图表
            showChart: function (formId) {
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                $.modal.loading("正在显示图表，请稍后...");
                $.post($.table._option.showChartUrl, $("#" + currentId).serializeArray(), function (result) {
                    if (result.code == web_status.SUCCESS) {
                        window.location.href = ctx + "common/showChart?fileName=" + result.msg + "&delete=" + true;
                    } else {
            ...
            selectDictLabel: function (_datas, _value) {
                ...
                $.each(_datas, function (index, dict) {
                    ...
                    if (dict.dictValue == _value) {
                        actions.push("<span class='badge badge-" + dict.listClass + "'>" + $.i18n.prop(dict.i18nKey) + "</span>");
                        return false;
                    }
                });
                return actions.join('');
        ...
        operate: {
            ...
            // 改变信息
            change: function (id) {
                var url = $.common.isEmpty(id) ? $.table._option.changeUrl : $.table._option.changeUrl.replace("{id}", id);
                $.modal.open($.i18n.prop("add") + $.table._option.modalName, url);
            },
            // 添加信息
            add: function (id) {
            ...
            edit: function (id) {
                ...
                $.modal.open($.i18n.prop("edit") + $.table._option.modalName, url);
            },
            // 添加信息 全屏
            addFull: function (id) {
                var url = $.common.isEmpty(id) ? $.table._option.createUrl : $.table._option.createUrl.replace("{id}", id);
            ...
            editFull: function (id) {
                var url = $.table._option.updateUrl.replace("{id}", id);
                $.modal.openFull($.i18n.prop("edit") + $.table._option.modalName, url);
            },
            // 保存信息
